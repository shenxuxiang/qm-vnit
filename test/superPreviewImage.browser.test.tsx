...
import SuperPreviewImage from '../src/lib/PreviewImage/SuperPreviewImage';
...
function Example(props: any) {
  ...
  return (
    <div>
      ...
      <SuperPreviewImage index={indictor} open={open} imgs={imgs} pageSize={9} onClose={() => setOpen(false)} />
    </div>
  );
}

describe('Test <SuperPreviewImage />', function() {
  test('Test <SuperPreviewImage /> Initial Render', async function() {
    ...
    // 确认 sliderBar 是否包含 10 个子节点
    expect(sliderBar.children).toHaveLength(10);
    // 确认 sliderBar 的第一个子节点为高亮
    expect(sliderBar.children[9]).toHaveClass('active');
    // 确认 sliderBar 位置是否是 { x: '-120px'; y: '0px'; z: '0px'  }
    expect(sliderBar).toHaveStyle({ transform: 'matrix(1, 0, 0, 1, -120, 0)' });
    ...
    expect(baseElement.querySelector('.qm-preview-image-foot-slider-list')).toHaveStyle({ transform: 'matrix(1, 0, 0, 1, -120, 0)' });
  ...
  test('Test <SuperPreviewImage /> Enlarge、Shrink、Rotate、Mirror', async function() {
    const user = userEvent.setup();
    const { baseElement, getByRole } = render(<Example indictor={-1}/>);
    ...
    expect(sliderBar.children[0]).toHaveClass('active', { exact: false });
    ...
    sliderBar = baseElement.querySelector('.qm-preview-image-foot-slider-list')!;
    // 确认 sliderBar 的第2个子节点为高亮
    expect(sliderBar.children[1]).toHaveClass('active');
  ...
  test('Test <SuperPreviewImage /> Mouse Wheel', async function() {
    const user = userEvent.setup();
  ...
  test('Test <SuperPreviewImage /> Drag', async function() {
    const user = userEvent.setup();
    ...
    console.log(baseElement.querySelector('.qm-preview-image-body-content'))
