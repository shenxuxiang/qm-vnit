 ...
 */
function SuperPreviewImage(props: SuperPreviewImageProps) {
  ...
  useEffect(() => {
    if (!open) return;
    const { url, hdUrl } = imgs[indictor];
    setState({ spinning: true, imageURL: url });
    const image = new Image();
    image.onload = () => setState({ spinning: false, imageURL: hdUrl });
    image.src = hdUrl;
    return () => {
      image.onload = null;
    };
  ...
  const smallImgs = useMemo(() => imgs.map((item) => item.url), [imgs]);
  ...
  return (
    <Modal open={open}>
      <div style={{ width: '100%', height: '100%' }}>
        ...
        {/* 底部滑块 */}
        <Slider imgs={smallImgs} onChange={handleChangeIndex} indictor={indictor} pageSize={pageSize} />
      </div>
    </Modal>
  );
}

export default memo(SuperPreviewImage);
